...
Laguerre polynomial PINN solver:
xy" + (1-x)y' + ny = 0, y = y(x)
y(0) = 1
y'(0) = -n

Tested for other simpler equations too
"""
...
def calculate_losses(model, x_c, x_bdy, n):
    ...
    # Loss for y(0) = 1
    tru_y_bdy_val = torch.ones_like(y_pred_bdy)
    bc_loss = torch.mean((y_pred_bdy - tru_y_bdy_val)**2)

    # Loss for y'(0) = -n
    dy_dx_bdy = torch.autograd.grad(y_pred_bdy, x_bdy, grad_outputs=torch.ones_like(y_pred_bdy), create_graph=True)[0]
    tru_y_deriv_bdy_val = -float(n) * torch.ones_like(dy_dx_bdy)
    deriv_bc_loss = torch.mean((dy_dx_bdy - tru_y_deriv_bdy_val)**2)
...
if __name__ == "__main__":
    ...
    n = 6             # Degree of the Laguerre polynomial
    ...
    epochs = 60000                  # Total training epochs
    ...
    num_collocation_points = 1500   # Number of points for ODE loss calculation
